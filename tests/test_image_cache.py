...
# client/client.py exits if onnxruntime cannot be imported; provide a tiny stub for unit tests.
if "onnxruntime" not in sys.modules:
    ...
    sys.modules["onnxruntime"] = _FakeOrtModule()
...
class AnimateDiffCapabilityTests(unittest.TestCase):
    ...
    def test_faceswap_metrics_include_cache_and_timing_keys_on_failure(self) -> None:
        entry = SimpleNamespace(name="epicrealismXL_vxviiCrystalclear")
        with patch.object(client_module, "read_gpu_stats", return_value={"utilization": 0}):
            orig_diffusers = client_module.diffusers
            try:
                client_module.diffusers = None  # type: ignore[assignment]
                metrics, _, _ = client_module._run_faceswap_task(
                    "job-fs-metrics",
                    entry,
                    Path("/tmp/base.safetensors"),
                    1.0,
                    {},
                )
            finally:
                client_module.diffusers = orig_diffusers

        self.assertIn("pipeline_cache_hit", metrics)
        self.assertIn("pipeline_load_ms", metrics)
        self.assertIn("face_detect_ms", metrics)
